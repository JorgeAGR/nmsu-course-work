...
upper = np.where(train_data[:,2] == 1)
lower = np.where(train_data[:,2] == -1)
...
for n, k in enumerate(k_neighbors):
    ...
    ax[n+1].set_xlim(-1.5, 2.5)
    ax[n+1].set_ylim(-1.5, 1.5)
...
ax[0].scatter(train_data[upper][:,0], train_data[upper][:,1], 100, marker='+', color='blue', label=r'target: $+1$')
ax[0].scatter(train_data[lower][:,0], train_data[lower][:,1], 100, marker='x', color='lightgreen', label=r'target: $-1$')
