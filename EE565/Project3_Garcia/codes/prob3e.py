...
data = gaussX(500, 0.2, seed=0)
x_train, y_train = data[:,:2], data[:,2]

perceptron = PerceptronClassifier()
perceptron.fit_Batch(x_train, y_train, seed=1)
...
blue_ind = y_right[np.where(y_right<(y_train==-1).sum())]
green_ind = y_right[np.where(y_right>=(y_train==-1).sum())]
