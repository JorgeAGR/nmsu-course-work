...
for i in range(len(all_nodes)):
    ax.text(all_nodes[i,0], all_nodes[i,1], names[i],
...
# Part B
# Feautre 1
sizes = np.asarray([som.codebook.matrix[:,2].reshape(nrows, ncols).T,
                    som.codebook.matrix[:,3].reshape(nrows, ncols).T])
...
ax2.text(2, 4, 'Medium', fontsize='medium', fontweight='bold',
         horizontalalignment='center', verticalalignment='center')
ax2.text(8, 11, 'Tall', fontsize='medium', fontweight='bold',
         horizontalalignment='center', verticalalignment='center')

# Feature 2
blueskin = np.asarray([som.codebook.matrix[:,7].reshape(nrows, ncols).T,
                   -som.codebook.matrix[:,7].reshape(nrows, ncols).T])
blueskin_labels = blueskin.argmax(axis=0)

ax3.axis('off')
ax3.scatter(xx, yy, u_matrix*250, c=blueskin_labels, cmap=cmap, 
            edgecolor='gray', marker='h', facecolors='none')
ax3.text(1, 1, 'Blue Skinned', fontsize='medium', fontweight='bold',
         horizontalalignment='center', verticalalignment='center')
ax3.text(6, 7, 'Other Colors', fontsize='medium', fontweight='bold',
         horizontalalignment='center', verticalalignment='center')
#fig3.tight_layout(pad=0.5)

# Feature 3
species = np.asarray([som.codebook.matrix[:,0].reshape(nrows, ncols).T,
                      som.codebook.matrix[:,1].reshape(nrows, ncols).T])
species_labels = species.argmax(axis=0)

ax4.axis('off')
ax4.scatter(xx, yy, u_matrix*250, c=species_labels, cmap=cmap, 
            edgecolor='gray', marker='h', facecolors='none')
ax4.text(8, 10, 'Reptilian', fontsize='medium', fontweight='bold',
         horizontalalignment='center', verticalalignment='center')
ax4.text(3, 6, 'Mammal', fontsize='medium', fontweight='bold',
         horizontalalignment='center', verticalalignment='center')
