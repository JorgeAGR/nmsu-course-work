# -*- coding: utf-8 -*-
"""
Created on Sun Oct  6 00:44:45 2019

@author: jorge
"""

import numpy as np
from requiredFunctions.maxMean import MaxMeanProjection
...
for i in range(2):
    for p, d in enumerate(data):    
        ...
        maxmean = MaxMeanProjection(train_x, train_y)
        maxmean.discriminant = maxmean.discriminant - i*maxmean.weights[0]
        accuracy = maxmean.score(train_x, train_y)
        ...
        cc = maxmean.predict(np.c_[xx0.ravel(), xx1.ravel()]).reshape(xx0.shape)
        ...
        means = np.array([train_x[train_y == 0].mean(axis=0), train_x[train_y == 1].mean(axis=0)])
        ...
        xnew_0 = maxmean.project(train_x[train_y == 0])
        xnew_1 = maxmean.project(train_x[train_y == 1])
        ax[1].hist(xnew_0, 40, color=cmap_scatter((0)))
        ax[1].hist(xnew_1, 40, color=cmap_scatter((1)))
        ax[1].axvline(maxmean.discriminant, color='black', linestyle='--', label='Decision')
        ...
        plt.savefig('../prob3'+part[p]+str(i)+'.eps', dpi=500)
